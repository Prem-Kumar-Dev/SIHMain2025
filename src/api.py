...
async def demo() -> Dict[str, Any]:
    ...
    return {
        "kpis": kpis,
        "schedule": [ScheduleItemOut(**vars(it)).dict() for it in schedule],
    }

@app.post("/schedule")
async def schedule(body: Dict[str, Any]) -> Dict[str, Any]:
    sections = [Section(**s) for s in body.get("sections", [])]
    trains = [TrainRequest(**t) for t in body.get("trains", [])]
    network = NetworkModel(sections=sections)
    schedule_items = schedule_trains(trains, network)
    kpis = summarize_schedule(schedule_items)
    return {
        "kpis": kpis,
        "schedule": [ScheduleItemOut(**vars(it)).dict() for it in schedule_items],
    }
